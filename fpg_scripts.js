...
jQuery(function($){
	$('#FPG_advanced_options').hide();
	$('#FPG_advanced_link').html('Show Advanced Options...');
	$('#FPG_advanced_link').click(function() {
		if ( 'Show Advanced Options...' == $(this).text() ) {
			$("#FPG_advanced_options").slideDown("normal");
			$(this).html('Hide Advanced Options...');
		}
		else {
			$('#FPG_advanced_options').slideUp("normal");
...
function image_url_sync(){
	...
	jQuery("#FPG_image_url_display").html(add_image_url);
}
...
function get_shortcode(filename) {
	if(filename == 'Null') {
		shortcode = 'No gallaries found.';
	} else {
		shortcode = '[fpg id="'+filename+'"]';
	}
	jQuery('#FPG_shortcode').html(shortcode);
}

function validate_edit(filename) {
	if (filename != 'Null') {
		if (jQuery('#FPG_file_do').val() == 'delete') return confirm('Are you sure you want to delete the gallery?');
	} else {
